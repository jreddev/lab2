document.getElementById("weatherSubmit").addEventListener("click", function(event) {
  ...
  fetch(url)
    .then(function(response) {
      ...
      let results = "";
           results += '<h2 class = "current">Current Weather in ' + json.name + "</h2>";
           for (let i=0; i < json.weather.length; i++) {
      results += '<img src="http://openweathermap.org/img/w/' + json.weather[i].icon + '.png"/>';
           }
           results += '<h2>' + json.main.temp + " &deg;F</h2>"
           results += "<p class = 'feels'>Feels like: " + json.main.feels_like + " &deg;F</p>";
           results += "<p class = 'wind_speeds'>Wind speed: " + json.wind.speed + "</p>";
           results += "<p>"
           for (let i=0; i < json.weather.length; i++) {
 ...
 fetch(url2)
   .then(function(response) {
     ...
     let forecast = "";
        ...
        forecast += "<p class = 'temp'>Temperature: " + json.list[i].main.temp + " &deg;F</p>";
        forecast += "<p class = 'feel'>Feels like: " + json.list[i].main.feels_like + " &deg;F</p>";
